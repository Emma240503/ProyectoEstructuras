...
def cargar_mapa(api):   #Recibe el api por parámetro
    ciudad_data = api.obtener_mapa()["data"]
    return ciudad_data["tiles"]

def dibujar_mapa(screen, tiles, colors, cam_x, cam_y, tile_size, view_width, view_height, imagenes=None):
    for y in range(cam_y, cam_y + view_height):
        for x in range(cam_x, cam_x + view_width):
            ...
            if imagenes and tile in imagenes:
                ...
                pygame.draw.rect(screen, colors.get(tile, (255, 0, 0)), (screen_x, screen_y, tile_size, tile_size))
