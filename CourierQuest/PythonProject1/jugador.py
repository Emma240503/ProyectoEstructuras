...
class Jugador:
    ...
    def calcular_multiplicador_velocidad(self, clima_mult, mapa_tiles):
        ...
        tile_actual = mapa_tiles[self.y][self.x] if 0 <= self.y < len(mapa_tiles) and 0 <= self.x < len(
            mapa_tiles[0]) else 'C'
        surface_weights = {'C': 1.0, 'P': 0.95, 'B': 0.0}  # Edificios bloqueados
        surface_weight = surface_weights.get(tile_actual, 1.0)

        velocidad_final = self.velocidad_base * clima_mult * Mpeso * Mrep * Mresistencia * surface_weight
    ...
    def recuperar(self):
        ...
        if ahora - self.ultimo_recupero >= 1:  # Cada segundo
            ...
            self.resistencia = min(self.max_resistencia, self.resistencia + puntos_recuperacion)
    ...
    def recoger_pedido(self, pedido):
        """Recoger un pedido si hay capacidad"""
        ...
        if self.peso_total() + pedido.weight <= self.capacidad:
            ...
            self.mensaje = f"Capacidad insuficiente (Peso necesario: {pedido.weight})"
            self.mensaje_tiempo = time.time()
            return False
    ...
    def cancelar_ultimo_pedido(self):
        ...
        if self.inventario:
            ...
            self.mensaje = f"Pedido cancelado (-4 reputación) Peso liberado: {pedido_cancelado.weight}"
            self.mensaje_tiempo = time.time()
    ...
    def entregar_pedido(self):
        """Entregar pedido con sistema de prioridades mejorado"""
        ...
        for p in pedidos_prioridad_max:
            if [self.x, self.y] == p.dropoff:
                ...
                if tiempo_transcurrido <= 20:  # Entrega puntual (≤20s)
                    if tiempo_transcurrido <= 16:  # Entrega temprana (≥20% antes de 20s)
                        self.reputacion = min(100, self.reputacion + 5)
                        self.entregas_tempranas += 1
                        self.mensaje = f"Entrega TEMPRANA! +5 reputación"
                    else:
                        self.reputacion = min(100, self.reputacion + 3)
                        self.mensaje = f"Entrega puntual +3 reputación"
                else:
                    # Entregas tardías con penalizaciones escaladas
                    if tiempo_transcurrido <= 50:  # 21-50s (equivalente a ≤30s en escala real)
                        self.reputacion = max(0, self.reputacion - 2)
                        self.mensaje = f"Entrega ligeramente tardía -2 reputación"
                    elif tiempo_transcurrido <= 140:  # 51-140s (equivalente a 31-120s)
                        self.reputacion = max(0, self.reputacion - 5)
                        self.mensaje = f"Entrega tardía -5 reputación"
                    else:  # >140s (equivalente a >120s)
                        self.reputacion = max(0, self.reputacion - 10)
                        self.mensaje = f"Entrega MUY tardía -10 reputación"
                ...
                # Sistema de rachas (bonus cada 3 entregas puntuales consecutivas)
                if hasattr(self, 'racha_entregas_puntuales'):
                    ...
                    self.racha_entregas_puntuales = 1 if tiempo_transcurrido <= 20 else 0
    ...
    def obtener_estadisticas(self):
        ...
        return {
            ...
            'eficiencia': self.entregas_completadas / max(1, self.entregas_completadas + self.cancelaciones),
            'puntualidad': self.entregas_tempranas / max(1, self.entregas_completadas)
        }
